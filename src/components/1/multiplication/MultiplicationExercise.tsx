...
export default function MultiplicationExercise({ num1, num2, correctAnswer, choices, onAnswer }: Props) {
  ...
  const [emoji] = useState(() => GROUP_EMOJIS[Math.floor(Math.random() * GROUP_EMOJIS.length)]);
  ...
  const getButtonClass = (value: number) => {
    ...
    if (selected === null) return base + 'bg-pink-200 hover:bg-pink-300 active:scale-95 text-pink-900';
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="flex gap-2 flex-wrap justify-center">
          {Array.from({ length: num1 }, (_, g) => (
            <div key={g} className="bg-pink-50 border-2 border-pink-300 rounded-2xl px-3 py-2 flex gap-1">
