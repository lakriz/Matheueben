...
import Division3Exercise from './Division3Exercise';
...
type ExerciseData = DivisionData | DrawingData;
...
function buildSession(): ExerciseData[] {
  // Generate from multiplication tables: divisor 2-10, result 2-20, dividend = divisor * result (max 100)
  const pool: DivisionData[] = [];
  for (let divisor = 2; divisor <= 10; divisor++) {
    for (let result = 2; result <= 20; result++) {
      ...
      pool.push({
        ...
        choices: generateChoices(result, 2, 20),
      });
    }
  }

  // Pick 12 random division exercises
  const divisions = shuffle(pool).slice(0, 12);
  ...
  // Interleave: 3 division, 1 drawing, repeat
  const result: ExerciseData[] = [];
  let di = 0;
  let dri = 0;
  while (di < divisions.length || dri < drawings.length) {
    if (di < divisions.length) result.push(divisions[di++]);
    if (di < divisions.length) result.push(divisions[di++]);
    if (di < divisions.length) result.push(divisions[di++]);
    if (dri < drawings.length) result.push(drawings[dri++]);
  }
  return result;
}
