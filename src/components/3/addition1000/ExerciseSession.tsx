...
import Addition1000Exercise from './Addition1000Exercise';
...
function buildSession(): ExerciseData[] {
  ...
  // 12 addition problems (grade 3: sums up to 1000)
  const additions: AdditionData[] = [];
  while (additions.length < 12) {
    const num1 = rand(10, 500);
    const num2 = rand(10, 500);
    if (num1 + num2 > 1000) continue;
    ...
    additions.push({
      ...
      choices: generateChoices(correct, Math.max(2, correct - 50), Math.min(1000, correct + 50)),
    });
  }
  ...
  // Interleave: 3 addition, 1 drawing, 3 addition, 1 drawing, …
