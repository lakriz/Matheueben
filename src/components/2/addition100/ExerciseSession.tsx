...
import Addition100Exercise from './Addition100Exercise';
...
function buildSession(): ExerciseData[] {
  ...
  // 12 addition problems (grade 2: sums up to 100)
  const additions: AdditionData[] = [];
  while (additions.length < 12) {
    const num1 = rand(1, 50);
    const num2 = rand(1, 50);
    ...
    additions.push({
      ...
      choices: generateChoices(correct, Math.max(2, correct - 10), Math.min(100, correct + 10)),
    });
  }
  ...
  // Interleave: 6 addition, 1 drawing, 6 addition, 1 drawing
  const result: ExerciseData[] = [];
  let ai = 0;
  let di = 0;
  while (ai < additions.length || di < drawings.length) {
    for (let k = 0; k < 6 && ai < additions.length; k++) result.push(additions[ai++]);
