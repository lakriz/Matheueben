...
import DivisionExercise from './DivisionExercise';
...
function buildSession(): ExerciseData[] {
  // Generate pairs from multiplication tables: divisor 2-10, result 1-10
  const allPairs: [number, number][] = [];
  for (let divisor = 2; divisor <= 10; divisor++) {
    for (let result = 1; result <= 10; result++) {
      allPairs.push([divisor * result, divisor]);
    }
  }

  // Pick 12 random pairs
  const chosen = shuffle(allPairs).slice(0, 12);
  const divisions: DivisionData[] = chosen.map(([dividend, divisor]) => {
    ...
    return {
      ...
      choices: generateChoices(correct, 1, 10),
    };
  });
  ...
  // Interleave: 6 division, 1 drawing, 6 division, 1 drawing
  const result: ExerciseData[] = [];
  let di = 0;
  let dri = 0;
  while (di < divisions.length || dri < drawings.length) {
    for (let k = 0; k < 6 && di < divisions.length; k++) result.push(divisions[di++]);
